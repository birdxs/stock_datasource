...
class TuShareRtIdxKPlugin(BasePlugin):
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        """Validate index K-line data."""
        if data.empty:
            self.logger.warning("Empty index K-line data")
            return False
        ...
        self.logger.info(f"Index K-line data validation passed for {len(data)} records")
        return True

    def transform_data(self, data: pd.DataFrame) -> pd.DataFrame:
        """Transform index K-line data."""
        ...
        self.logger.info(f"Transformed {len(data)} index K-line records")
        return data

    def load_data(self, data: pd.DataFrame) -> Dict[str, Any]:
        """Load index K-line data into ClickHouse."""
        if not self.db:
        ...
        try:
            ...
            # Insert data
            self.db.insert_dataframe('ods_rt_idx_k', data)
            
            self.logger.info(f"Successfully loaded {len(data)} index K-line records")
            return {
                "status": "success",
            ...
            self.logger.error(f"Failed to load index K-line data: {e}")
