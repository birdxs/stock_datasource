"""TuShare rt_etf_min (ETF实时分钟K线) data extractor."""
...
class RtEtfMinExtractor:
    """Extractor for TuShare rt_min (ETF实时分钟K线) data."""
    ...
    def _call_api(self, ts_code: str, freq: str) -> pd.DataFrame:
        """Call TuShare rt_min API for ETF with rate limiting and retry."""
        ...
        try:
            ...
            if result is None or result.empty:
                logger.warning(f"API returned empty data for ETF ts_code={ts_code}, freq={freq}")
                return pd.DataFrame()
            
            logger.info(f"API call successful: ETF ts_code={ts_code}, freq={freq}, records={len(result)}")
            return result
            
        except Exception as e:
            logger.error(f"API call failed: ETF ts_code={ts_code}, freq={freq}, error={e}")
            raise
    ...
    def extract(self, ts_code: str, freq: str = "1MIN") -> pd.DataFrame:
        ...
        Args:
            ts_code: ETF code(s), supports comma-separated (e.g., '589960.SH,159100.SZ')
        ...
        logger.info(f"Extracting rt_etf_min: ts_code={ts_code}, freq={freq}")
        
        df = self._call_api(ts_code, freq)
        
        if not df.empty:
            df = self._process_data(df, freq)
            logger.info(f"Extracted {len(df)} ETF minute records")
        else:
            logger.warning("No ETF minute data extracted")
            
        return df
    
    def extract_batch(self, ts_codes: List[str], freq: str = "1MIN", batch_size: int = 10) -> pd.DataFrame:
        """Extract minute data for multiple ETFs in batches."""
        all_data = []
        ...
        for i in range(0, len(ts_codes), batch_size):
            ...
            try:
                ...
                logger.error(f"Failed to extract ETF batch {i//batch_size}: {e}")
